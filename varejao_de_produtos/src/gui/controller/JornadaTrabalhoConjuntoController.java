	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		buttonSalvarJornada.setOnAction(new EventHandler<ActionEvent>(){
			...
			public void handle(ActionEvent event) {
				...
				Alert alert = new Alert(AlertType.ERROR);
				alert.setTitle("Erro ao cadastrar uma Jornada.");
				alert.setHeaderText("Parametros inv�lidos ou nulos.");
				alert.setContentText(e.getMessage());
				alert.showAndWait();
			}
		...
		btSalvarPeriodo.setOnAction(new EventHandler<ActionEvent>(){
			...
			public void handle(ActionEvent event) {
				...
				try{
					if(event.getSource()==btSalvarPeriodo){
							...
							try {
								...
								Alert alert = new Alert(Alert.AlertType.INFORMATION);
								alert.setTitle("Tela Cadastro Jornada e Periodo");
								alert.setHeaderText("Cadastramento Per�odo");
								alert.setContentText("Per�odo cadastrado com sucesso");
								alert.show();
							} catch (Exception e) {
								// TODO Auto-generated catch block
								Alert alert = new Alert(AlertType.ERROR);
								alert.setTitle("Erro ao cadastrar um Per�odo.");
								alert.setHeaderText("Impossivel efetuar cadastro.");
								alert.setContentText(e.getMessage());
								alert.showAndWait();
							}
				...
				Alert alert = new Alert(AlertType.ERROR);
				alert.setTitle("Erro ao cadastrar um Per�odo.");
				alert.setHeaderText("Parametros inv�lidos ou nulos, imposs�vel realizar a��o.");
				alert.setContentText(e.getMessage());
				alert.showAndWait();
			}
				...
				try{
					if(event.getSource()==btDeletarJornada){
						...
						try {
							...
							alert.setHeaderText("Parametros inv�lidos ou nulos. Selecione uma jornada para deletar");
							alert.setContentText(e.getMessage());
							alert.showAndWait();
						}
